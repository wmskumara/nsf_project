...
begin = True
data_file = '/home/pi/Desktop/ADC2/ABElectronics_Python3_Libraries-master/ADCDifferentialPi/data_file.csv'
...
ss2 = [None]*3
sr1 = sensor_data()
file = open('/home/pi/Desktop/ADC2/ABElectronics_Python3_Libraries-master/ADCDifferentialPi/moisture_out/real_time_moisture.txt', 'w')#open file to write data step by step 'w' denotes write


def calK(temp, rh):#call when needed
   tc = temp #inlet temperature in celcius
   tx = (tc *9)/5 + 32
   RH = rh #relative humidity as a %
   ta = float(tc) + 273.15 #float use because from file read as a (sr1.find_trueRH_sr1(adc.read_voltage(2),tc))/100
...
with open('/home/pi/Desktop/ADC2/ABElectronics_Python3_Libraries-master/ADCDifferentialPi/data_file.csv', 'a') as output:   
	while (True):   
		temp1 = sr1.find_true_temp(adc.read_voltage(1))
		print("Sensor 1 temp :", round(temp1,2))   
		RH1 = sr1.find_trueRH_sr1(adc.read_voltage(2),temp1)
		print("Sensor 1 RH :", round(RH1,2))
		temp2 = sr1.find_true_temp(adc.read_voltage(3))
		print("Sensor 2 temp :", round(temp2,2))
		RH2=sr1.find_trueRH_sr2(adc.read_voltage(4),temp2)
		print("Sensor 2 RH :", round(RH2,2))
		#print("Temp", temp1)
		#print("RH1", RH1)
		#print("RH2", RH2)
		if temp1 is not None and RH1 is not None and RH2 is not None:
			avg_temp = round((temp1 + temp2) / 2,1)
			print("Average temp :", avg_temp)
			avg_RH = round((RH1 + RH2) / 2,1)
			print("Average RH :", avg_RH)
			...
			for l in range(0,3):
				...
				total += mmwb;
			average = total/3
			hy_diff = calWB(avg_temp, avg_RH)
			txt = time.strftime("%H:%M:%S") + '\t' + str(round(average,2)) + '\t' + str(avg_temp)+ '\t' + str(avg_RH) + ' \n'
			file.write(txt)
			data = [time.strftime("%H:%M:%S"),avg_temp,avg_RH,round(average,3)]
			...
			#print("TEmprature :", round(temp1,0))
